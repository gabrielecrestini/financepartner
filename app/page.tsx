...
export default function ProtocolloSoftware() {
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-200 font-sans relative flex flex-col items-center py-6 sm:py-10 px-4 sm:px-6">
      ...
      <div className="absolute top-0 w-full max-w-[800px] h-[400px] bg-indigo-900/10 blur-[100px] pointer-events-none"></div>
      ...
      <div className="max-w-6xl w-full z-10">
        ...
        {/* STEPS 1-3 */}
        {step === 1 && (
          <div className="bg-slate-900/80 p-6 sm:p-12 rounded-3xl border border-slate-800 max-w-xl mx-auto text-center animate-in zoom-in-95">
        ...
        {step === 2 && (
          <div className="bg-slate-900/80 p-6 sm:p-12 rounded-3xl border border-slate-800 max-w-xl mx-auto text-center animate-in slide-in-from-right-8">
        ...
        {step === 3 && (
          <div className="bg-slate-900/80 p-8 sm:p-10 rounded-3xl border border-cyan-500/20 max-w-md mx-auto text-center animate-in fade-in">
            <Settings size={40} className="text-cyan-400 mx-auto mb-6 animate-spin-slow" />
            <h3 className="text-lg sm:text-xl font-bold text-white mb-4">Compilazione Ambiente...</h3>
            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden mb-4"><div className="bg-cyan-500 h-full transition-all duration-300" style={{ width: `${progress}%` }}></div></div>
        ...
        {step === 4 && !isAdmin && (
          <div className="animate-in zoom-in-95 duration-500">
            ...
            {/* STATO 1: Unregistered */}
            {userStatus === 'unregistered' && (
              <div className="max-w-3xl mx-auto bg-slate-900/80 p-6 sm:p-12 rounded-3xl border border-slate-800 text-center shadow-2xl">
                <AlertTriangle size={40} className="text-amber-400 mx-auto mb-4 sm:mb-6" />
                <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">Inizializzazione Nodi</h2>
                <p className="text-sm sm:text-base text-slate-400 mb-6 sm:mb-8">Per accedere all'Accademia Istituzionale e ai calcolatori, verifica la connessione attiva con i due broker a zero fee.</p>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 text-left mb-6">
                  <a href={LINK_YOUHODLER} target="_blank" rel="noreferrer" onClick={() => handleNodeClick(1)} className={`p-5 sm:p-6 rounded-2xl border transition-all ${node1Clicked ? 'bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'bg-slate-950 border-slate-700 hover:border-cyan-500'}`}>
                    <div className="text-[10px] sm:text-xs font-black tracking-widest text-cyan-400 mb-2">STEP 1 - OBBLIGATORIO</div>
                    <div className="text-white font-bold text-base sm:text-lg mb-3 sm:mb-4">Nodo YouHodler (10%)</div>
                  ...
                  <a href={LINK_ETORO} target="_blank" rel="noreferrer" onClick={() => handleNodeClick(2)} className={`p-5 sm:p-6 rounded-2xl border transition-all ${node2Clicked ? 'bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'bg-slate-950 border-slate-700 hover:border-indigo-500'}`}>
                    <div className="text-[10px] sm:text-xs font-black tracking-widest text-indigo-400 mb-2">STEP 2 - OBBLIGATORIO</div>
                    <div className="text-white font-bold text-base sm:text-lg mb-3 sm:mb-4">Nodo eToro (Equity)</div>
                ...
                {node1Clicked && node2Clicked && (
                  <div className="text-emerald-400 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 animate-pulse"><RefreshCw size={16} className="animate-spin"/> Sincronizzazione in corso...</div>
                )}
              </div>
            )}
            ...
            {/* STATO 2: Pending */}
            {userStatus === 'pending' && (
              <div className="max-w-2xl mx-auto bg-slate-900/80 p-8 sm:p-12 rounded-3xl border border-indigo-500/30 text-center shadow-2xl relative overflow-hidden">
                ...
                <Lock size={48} className="text-indigo-400 mx-auto mb-4 sm:mb-6" />
                <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">Auditing in Corso</h2>
                <p className="text-sm sm:text-base text-slate-400 mb-6 sm:mb-8 leading-relaxed">
                  I nodi sono stati rilevati in locale. Il nostro team di conformità sta verificando manualmente l'handshake delle API per garantirti l'accesso alla piattaforma privata (Valore 10.000€).
                </p>
                
                <div className="bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800 text-center flex flex-col items-center">
                  <div className="text-slate-500 text-xs sm:text-sm mb-4">Attivazione stimata: 24-48h. Se hai ricevuto conferma, forza l'aggiornamento.</div>
                  <button onClick={() => checkVerification(email, true)} disabled={isChecking} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-6 sm:px-8 rounded-xl flex items-center justify-center gap-2 transition-all w-full shadow-lg shadow-indigo-500/20 text-sm sm:text-base">
                    {isChecking ? <RefreshCw className="animate-spin" size={18}/> : <Shield size={18}/>}
                    {isChecking ? "CONTROLLO DATABASE..." : "VERIFICA STATO"}
                  </button>
                </div>
              </div>
            )}
            ...
            {userStatus === 'verified' && (
              <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                ...
                {/* Sidebar VIP (Menu Orizzontale Scrollabile su Mobile) */}
                <div className="w-full lg:w-64 flex flex-col gap-2 shrink-0">
                  <div className="bg-slate-900 p-4 sm:p-6 rounded-2xl border border-emerald-500/30 mb-2 lg:mb-4">
                    <div className="text-[10px] sm:text-xs text-emerald-400 font-bold tracking-widest mb-1 uppercase">Stato Account</div>
                    <div className="text-white font-black flex items-center gap-2 text-sm sm:text-base"><CheckCircle size={16}/> ACCESSO VIP</div>
                  </div>
                  
                  {/* Menu Navigazione */}
                  <div className="flex overflow-x-auto lg:flex-col gap-2 pb-2 lg:pb-0 hide-scrollbar snap-x">
                    <button onClick={() => setVipTab('dashboard')} className={`p-3 sm:p-4 rounded-xl flex items-center justify-center lg:justify-start gap-2 sm:gap-3 font-bold transition-all shrink-0 snap-center text-sm sm:text-base ${vipTab === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900 text-slate-400 hover:bg-slate-800 border border-slate-800'}`}>
                      <LineChart size={18}/> Calcolatore
                    </button>
                    <button onClick={() => setVipTab('copytrading')} className={`p-3 sm:p-4 rounded-xl flex items-center justify-center lg:justify-start gap-2 sm:gap-3 font-bold transition-all shrink-0 snap-center text-sm sm:text-base ${vipTab === 'copytrading' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900 text-slate-400 hover:bg-slate-800 border border-slate-800'}`}>
                      <TrendingUp size={18}/> CopyTrading
                    </button>
                    <button onClick={() => setVipTab('loans')} className={`p-3 sm:p-4 rounded-xl flex items-center justify-center lg:justify-start gap-2 sm:gap-3 font-bold transition-all shrink-0 snap-center text-sm sm:text-base ${vipTab === 'loans' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900 text-slate-400 hover:bg-slate-800 border border-slate-800'}`}>
                      <Repeat size={18}/> Leva
                    </button>
                    <button onClick={() => setVipTab('chat')} className={`p-3 sm:p-4 rounded-xl flex items-center justify-center lg:justify-start gap-2 sm:gap-3 font-bold transition-all shrink-0 snap-center text-sm sm:text-base ${vipTab === 'chat' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-900 text-slate-400 hover:bg-slate-800 border border-slate-800'}`}>
                ...
                <div className="flex-grow bg-slate-900 rounded-3xl border border-slate-800 p-5 sm:p-8 md:p-10 min-h-[500px] shadow-2xl relative overflow-hidden">
                  ...
                  {vipTab === 'dashboard' && (
                    <div className="animate-in fade-in">
                      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3"><LineChart className="text-indigo-400" size={24}/> Proiezione & Automazione</h2>
                      
                      {/* Mini Calcolatore */}
                      <div className="bg-slate-950 p-5 sm:p-6 rounded-2xl border border-slate-800 mb-6 sm:mb-8">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                           <div className="space-y-2">
                             <label className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase">Capitale (€)</label>
                             ...
                             <div className="text-lg sm:text-xl font-bold text-white">€ {capital.toLocaleString()}</div>
                           </div>
                           <div className="space-y-2">
                             <label className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase">Anni</label>
                             ...
                             <div className="text-lg sm:text-xl font-bold text-white">{anni} Anni</div>
                           </div>
                        </div>
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 sm:p-4 bg-slate-900 rounded-xl border border-slate-800">
                           <div className="text-xs sm:text-sm text-slate-400">Rendimento Stimato (10%):</div>
                           <div className="text-xl sm:text-2xl font-black text-emerald-400">€ {guadagnoNetto.toLocaleString('it-IT', {maximumFractionDigits:0})}</div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800">
                        <ul className="list-disc pl-4 text-xs sm:text-sm text-slate-400 space-y-3 leading-relaxed">
                          ...
                          <li>Nelle impostazioni di YouHodler, attiva la conversione automatica da EUR a EURS per avviare istantaneamente la rendita.</li>
                          <li>Su eToro, collega il conto bancario tramite Addebito Diretto (SEPA). Imposta un prelievo automatico il giorno 5 equivalente agli interessi mensili generati per acquistare quote di SPY (Indice S&P 500).</li>
                        </ul>
                      </div>
                    </div>
                  )}
                  ...
                  {vipTab === 'copytrading' && (
                    <div className="animate-in fade-in">
                      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3"><TrendingUp className="text-indigo-400" size={24}/> CopyTrading Asimmetrico</h2>
                      <p className="text-sm sm:text-base text-slate-300 mb-6 leading-relaxed">Sfrutta l'intelligenza collettiva del Nodo 2 (eToro) mettendo a leva i profitti generati dal Nodo 1. Affida le tue plusvalenze ai migliori trader.</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div className="bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800">
                          <div className="text-sm sm:text-base text-indigo-400 font-bold mb-2">Il Filtro Alpha</div>
                          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">Evita i giocatori d'azzardo. Nella ricerca di eToro imposta: Rischio Max 3, Storicità oltre i 2 anni, e Drawdown (perdita massima) inferiore al 10% annuo.</p>
                        </div>
                        <div className="bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800">
                          <div className="text-sm sm:text-base text-indigo-400 font-bold mb-2">Pac Mensile (DCA)</div>
                          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">Non versare tutto in una volta. Aggiungendo liquidità al copytrader ogni mese, i suoi mesi negativi diventeranno sconti per acquistare nuovi asset a prezzo ribassato.</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {vipTab === 'loans' && (
                    <div className="animate-in fade-in">
                      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-6 flex items-center gap-2 sm:gap-3"><Repeat className="text-indigo-400" size={24}/> Leva via Prestiti</h2>
                      <div className="bg-rose-500/10 border border-rose-500/30 p-5 sm:p-6 rounded-xl mb-4 sm:mb-6">
                        ...
                        <p className="text-xs sm:text-sm text-rose-300 mt-2">Modulo ad alto rischio. Usa i prestiti collaterali per estrarre contanti senza vendere gli asset principali che stanno maturando il 10%.</p>
                      </div>
                      <div className="bg-slate-950 p-5 sm:p-6 rounded-xl border border-slate-800">
                        ...
                        <p className="text-slate-400 text-xs sm:text-sm leading-relaxed">
                          <strong>Strategia Base:</strong> Prendi in prestito il 30% del collaterale (LTV Sicuro). Invialo su eToro per sfruttare un crollo azionario improvviso (es. un flash crash del -15%). Quando il mercato recupera, vendi le azioni su eToro, estingui il prestito su YouHodler e intasca la plusvalenza netta.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {vipTab === 'chat' && (
                    <div className="absolute inset-0 p-4 sm:p-6 md:p-10 flex flex-col animate-in fade-in bg-slate-900">
                      <div className="flex-shrink-0 mb-4">
                        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2 flex items-center gap-2 sm:gap-3"><Users className="text-indigo-400" size={24}/> La War Room</h2>
                        <p className="text-slate-500 text-xs sm:text-sm">Spazio riservato esclusivamente agli utenti verificati.</p>
                      </div>
                      
                      <div className="flex-grow bg-slate-950 rounded-xl border border-slate-800 p-3 sm:p-4 mb-4 overflow-y-auto flex flex-col gap-3 min-h-[250px]">
                        {chatMessages.length === 0 ? (
                          <div className="text-center text-slate-500 my-auto text-sm">Nessun messaggio presente. Avvia la discussione.</div>
                        ) : (
                          chatMessages.map((msg, i) => (
                            <div key={i} className={`p-3 sm:p-4 rounded-xl max-w-[90%] sm:max-w-[85%] ${msg.user_email === email ? 'bg-indigo-600/20 border-indigo-500/30 ml-auto' : 'bg-slate-800 border-slate-700'} border shadow-sm`}>
                              <div className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1.5 sm:mb-2 flex items-center gap-1.5 sm:gap-2">
                                <Shield size={10}/> {msg.user_email.split('@')[0]}
                              </div>
                      ...
                      <form onSubmit={sendChatMessage} className="flex-shrink-0 flex gap-2">
                        <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Scrivi al gruppo..." className="flex-grow p-3 sm:p-4 bg-slate-950 border border-slate-700 rounded-xl text-white outline-none focus:border-indigo-500 shadow-inner text-sm" />
                        <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 sm:px-8 rounded-xl transition-all flex items-center justify-center font-bold shadow-lg"><Send size={18}/></button>
                      </form>
                    </div>
                  )}
      ...
      {/* CSS Utilities inlinea per nascondere la scrollbar su mobile */}
